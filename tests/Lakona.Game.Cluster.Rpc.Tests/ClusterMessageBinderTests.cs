...
public sealed class ClusterMessageBinderTests
{
    [Fact]
    public async Task BinderRegistersClusterSendHandlerAndReturnsDispatchStatus()
    {
        ...
        ClusterMessageBinder.Bind(registry, handler);
        ...
        using var payload = serializer.SerializeFrame(new ClusterSendRequest
        {
            Route = "room/1",
            Kind = "command",
        ...
        var reply = serializer.Deserialize<ClusterSendReply>(response.Payload.Memory);
