...
public sealed class ClusterFormationCoordinatorTests
{
    ...
    public async Task Membership_frame_binder_returns_ok_with_unavailable_outcome_during_formation()
    {
        ...
        Assert.True(registry.TryGetHandler(ClusterProtocol.ServiceId, ClusterProtocol.Methods.MembershipFrame, out var handler));
        ...
        using var responseFrame = await handler!(session, new RpcRequestFrame(1, ClusterProtocol.ServiceId, ClusterProtocol.Methods.MembershipFrame, payload), TestContext.Current.CancellationToken);
    ...
    public async Task Membership_frame_binder_classifies_invalid_typed_requests_as_bad_request_without_invoking_handler()
    {
        ...
        foreach (var payload in invalidPayloads)
        {
            using var request = RpcEnvelopeCodec.EncodeRequest(new RpcRequestEnvelope
            {
                ...
                MethodId = ClusterProtocol.Methods.MembershipFrame,
                Payload = payload
            });
