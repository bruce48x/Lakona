...
public sealed class StarterTemplateGeneratorTests
{
    ...
    {
        var repositoryRoot = FindRepositoryRoot();
        var readme = File.ReadAllText(Path.Combine(repositoryRoot, "src", "Lakona.Rpc.Starter", "README.md"));

        Assert.Contains("Generated RPC glue is compiler output.", readme);
        Assert.Contains("Lakona.Rpc.Analyzers", readme);
        ...
        Assert.DoesNotContain("lakona-starter codegen", readme, StringComparison.OrdinalIgnoreCase);
