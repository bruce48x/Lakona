...
[CollectionDefinition(TimerTestCollectionNames.TimerRuntime, DisableParallelization = true)]
public sealed class TimerSchedulerCollection;

internal static class TimerTestCollectionNames
{
    public const string TimerRuntime = "Lakona timer runtime tests";
}
...
public sealed class LakonaTimerSchedulerTests : IDisposable
{
    ...
    public async Task Repeated_partial_advances_while_scheduler_arms_delay_preserve_original_due_time()
    {
        ...
        try
        {
            ...
            await observer.WaitForQueuedAsync(cancellationToken);
            await TimerCallbackLog.WaitForValueAsync("partial-arm-race", cancellationToken);
        }
        finally
        {
            time.ReleaseTimerCreation(1);
