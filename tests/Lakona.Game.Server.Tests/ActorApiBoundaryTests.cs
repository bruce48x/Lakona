...
public sealed class ActorApiBoundaryTests
{
    ...
    public void Game_server_readme_teaches_generated_selectors_before_raw_runtime()
    {
        ...
        Assert.Contains("await rooms.Get(roomId).JoinAsync", readme, StringComparison.Ordinal);
        Assert.Contains("await rooms.Local(roomId).JoinAsync", readme, StringComparison.Ordinal);
        Assert.Contains("await rooms.Remote(nodeId, roomId).JoinAsync", readme, StringComparison.Ordinal);
