...
public sealed class HotfixGeneratorTests
{
    ...
    public void Exception_components_are_rejected_without_generating_registration(bool indirect)
    {
        var result = GeneratorTestHost.Run($$"""
            ...
            """);
        Assert.Contains(result.GeneratorDiagnostics, diagnostic => diagnostic.Id == "LAKONA20060");
    ...
    public void Actor_timer_marker_requires_a_behavior_and_valid_signature()
    {
        var result = GeneratorTestHost.Run("""
            ...
            """);
        Assert.Contains(result.GeneratorDiagnostics, diagnostic => diagnostic.Id == "LAKONA20049");
    }
    ...
    public void Generator_rejects_an_empty_actor_method_wire_name()
    {
        ...
        Assert.Contains(
            ...
            diagnostic => diagnostic.Id == "LAKONA20046");
    }
    ...
    public void Generator_rejects_actor_method_and_actor_ignore_on_the_same_method()
    {
        ...
        Assert.Contains(
            result.Hotfix.GeneratorDiagnostics,
            diagnostic => diagnostic.Id == "LAKONA20047");
    }
    ...
    public void Generator_emits_hotfix_actor_refs_from_behavior_methods_without_contract()
    {
        ...
        Assert.Contains("public static global::System.Threading.Tasks.ValueTask<global::Game.Server.LoginReply> CallAsync(", generated, StringComparison.Ordinal);
        Assert.Contains("global::System.Func<global::Game.Hotfix.UserBehavior, global::Lakona.Game.Server.Hotfix.Abstractions.Actors.HotfixActorCall<global::Game.Server.UserActor, global::Game.Server.LoginRequest, global::Game.Server.LoginReply>> selector", generated, StringComparison.Ordinal);
        Assert.Contains("public static global::System.Threading.Tasks.ValueTask PostAsync(", generated, StringComparison.Ordinal);
        Assert.DoesNotContain("public static class Entries", generated, StringComparison.Ordinal);
    ...
    [Fact]
    public void Generated_actor_access_supports_actor_only_type_arguments_and_instance_method_selector_inference()
    {
        var appSource = """
            using System.Runtime.CompilerServices;
            using Lakona.Game.Server.Actors;
        ...
        var hotfixSource = """
            ...
            internal sealed class Caller(ActorAccess actors)
            {
                ...
                {
                    return actors.Route<UserActor>(userId).CallAsync(
    ...
    public void Generator_reports_duplicate_generated_behavior_method_signature()
    {
        ...
        AssertContainsBehaviorDiagnostic(
            ...
            "duplicate",
            "generated");
        Assert.Contains(result.Hotfix.GeneratorDiagnostics, diagnostic => diagnostic.Id == "LAKONA20030");
    ...
    [Fact]
    public void Generator_emits_actor_selectors_for_behavior_methods()
    {
        var appSource = """
            using System.Runtime.CompilerServices;
            using System.Threading;
        ...
        Assert.Empty(result.App.ErrorDiagnostics);
        Assert.Empty(result.Hotfix.ErrorDiagnostics);
        Assert.DoesNotContain("public sealed partial class UserBehavior", generated, StringComparison.Ordinal);
        Assert.Contains("public sealed class ActorAccess", generated, StringComparison.Ordinal);
        Assert.Contains("public LocalActor<TActor> Local<TActor>(global::Game.Server.UserId id)", generated, StringComparison.Ordinal);
        Assert.Contains("public ActorRoute<TActor> Route<TActor>(global::Game.Server.UserId id)", generated, StringComparison.Ordinal);
    ...
    public void Generator_emits_local_and_route_actor_refs_without_business_wrappers()
    {
        ...
        Assert.Empty(result.Hotfix.ErrorDiagnostics);
        Assert.DoesNotContain("internal sealed partial class RoomBehavior", generated, StringComparison.Ordinal);
        Assert.Contains("public readonly struct LocalActor<TActor>", generated, StringComparison.Ordinal);
        Assert.Contains("public readonly struct ActorRoute<TActor>", generated, StringComparison.Ordinal);
        Assert.Contains("global::System.Func<global::Game.Hotfix.Rooms.RoomBehavior", generated, StringComparison.Ordinal);
    ...
    public void Generator_reports_duplicate_hotfix_behaviors_without_throwing()
    {
        ...
        Assert.Contains(result.Hotfix.GeneratorDiagnostics, diagnostic => diagnostic.Id == "LAKONA20018");
        Assert.DoesNotContain("this global::Game.Server.UserRef self", result.Hotfix.GeneratedSource, StringComparison.Ordinal);
    }
    ...
    public void Generator_reports_nested_hotfix_behavior_without_generating_actor_ref_extensions()
    {
        ...
        Assert.Contains(result.Hotfix.GeneratorDiagnostics, diagnostic => diagnostic.Id == "LAKONA20021");
        Assert.DoesNotContain("this global::Game.Server.UserRef self", result.Hotfix.GeneratedSource, StringComparison.Ordinal);
    }
    ...
    public void Generator_excludes_actor_lifecycle_methods_from_actor_api()
    {
        ...
        Assert.DoesNotContain(result.GeneratorDiagnostics, diagnostic => diagnostic.Id == "LAKONA20028");
        Assert.Empty(result.ErrorDiagnostics);
    ...
    [Fact]
    public void Hotfix_http_service_validates_without_app_owned_registration()
    {
        var source = """
            using System.Threading.Tasks;
    ...
    [Fact]
    public void Hotfix_http_service_rejects_management_route()
    {
        var source = """
            using System.Threading.Tasks;
            using Lakona.Game.Server.Http;
            ...
            {
                [LakonaHttpEndpoint("GET", "/_lakona/secrets")]
                public ValueTask<LakonaHttpResponse> ReadAsync(LakonaHttpCall call)
                {
                    return new ValueTask<LakonaHttpResponse>(
                        LakonaHttpResponse.Text("no"));
                }
            }
            """;
        ...
        Assert.Contains(
            ...
            static diagnostic => diagnostic.Id == "LAKONA20041");
    }

    [Fact]
    public void Hotfix_http_service_rejects_duplicate_route_without_method_ids()
    {
        var source = """
            using System.Threading.Tasks;
            using Lakona.Game.Server.Http;
            ...
            public sealed class OperationsService
            {
                [LakonaHttpEndpoint("GET", "/users/{account}")]
                public ValueTask<LakonaHttpResponse> FirstAsync(LakonaHttpCall call)
                {
                    return new ValueTask<LakonaHttpResponse>(
                        LakonaHttpResponse.Text("first"));
                }

                [LakonaHttpEndpoint("get", "/USERS/{ACCOUNT}")]
                public ValueTask<LakonaHttpResponse> SecondAsync(LakonaHttpCall call)
                {
                    return new ValueTask<LakonaHttpResponse>(
                        LakonaHttpResponse.Text("second"));
                }
            }
            """;
        ...
        Assert.Contains(
            result.GeneratorDiagnostics,
            static diagnostic => diagnostic.Id == "LAKONA20045");
    }
    ...
    [Theory]
    [InlineData("[RpcService(1)] public interface IMissingRpcMethod { ValueTask PingAsync(Request request); }", "LAKONA20007")]
    [InlineData("[RpcService(1)] public interface ITwoParameters { [RpcMethod(1)] ValueTask PingAsync(Request request, Request other); }", "LAKONA20008")]
    [InlineData("[RpcService(1)] public interface IUnsupportedReturn { [RpcMethod(1)] Task PingAsync(Request request); }", "LAKONA20009")]
    [InlineData("[RpcService(1, NotificationContract = typeof(BadCallback))] public interface IBadCallbackService { [RpcMethod(1)] ValueTask PingAsync(Request request); } public sealed class BadCallback { }", "LAKONA20010")]
