...
`Lakona.Rpc.Analyzers` generates a project-specific `RpcClient` facade through a Roslyn Source Generator. It is not a fixed type from a NuGet package; it is generated during the current project compilation.
...
`RpcClient` internally holds `Lakona.Rpc.Client.RpcClientRuntime`.
...
Generated clients, service proxies, notification binders, and server binders are generated by `Lakona.Rpc.Analyzers` at compile time:
