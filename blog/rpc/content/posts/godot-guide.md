...
`Client.csproj` references `../Shared/Shared.csproj` and adds NuGet packages for the selected transport and serializer. Godot client RPC glue is generated by `Lakona.Rpc.Analyzers` at compile time.
...
In MemoryPack mode, Shared DTOs include MemoryPack annotations and the starter writes the related dependencies into the project. DTO versioning needs more care; see [DTO Versioning](/Lakona.Rpc/posts/dto-versioning/).
...
Lakona.Rpc background callbacks are not guaranteed to run on the Godot main thread. When updating Node, SceneTree, or UI objects, hand results off to main-thread logic.
