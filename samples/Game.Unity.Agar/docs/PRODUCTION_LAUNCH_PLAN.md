# Game.Unity.Agar 生产上线规划

本文面向 `samples/Game.Unity.Agar` 从“可演示样例”推进到“小规模生产上线”的评审。本文回答上线前还缺哪些功能、还需要做哪些工程和验证工作。
...
- 联机采用 20Hz 确定性帧同步；服务端只排序、保存并转发输入帧，客户端运行完整战斗模拟。
...
当前样例已经具备核心玩法和联机骨架：共享确定性模拟内核、帧同步启动/输入/重放协议、WebSocket 控制面、KCP 实时面、匹配与房间分配、按会话隔离的可靠匹配推送、PostgreSQL 状态保存、胜利积分和 Redis 排行榜都已接入。

距离生产上线的主要差距不在“有没有基本玩法”，而在以下几类：

- 帧历史可支持当前回合内重连，但尚缺状态摘要、跨客户端 desync 检测和长时间断线策略。
- 服务端不计算战斗，因此无法独立验证客户端上报的排名；面向不可信公网玩家前必须明确反作弊取舍。
...
| 模块 | 当前基础 | 上线前缺口 | 首发处理 |
| --- | --- | --- | --- |
| 登录与会话 | 已有登录、游客登录、重连参数、token 和 `Lakona.Game.Server.Sessions` 会话身份 | 缺少异常状态的玩家级提示、重复登录策略、token 生命周期和生产回归 | 必须补齐 |
| 匹配 | 已有匹配队列和按会话隔离的可靠匹配推送 | 取消匹配、断线重连、匹配超时、重复点击的最终体验需要回归 | 必须补齐 |
| 实时对局 | 已有输入提交、20Hz 组帧、有界重放、客户端确定性模拟和结果上报 | 缺少 desync 摘要/告警；客户端结果信任模型不适合强对抗公网竞技 | 必须补齐 |
...
当前 room actor 是输入序列和帧历史的单一 owner；实时 attach 被路由到该 owner 所在 battle 节点，RPC 回调对象和帧扇出保持进程本地。控制连接可以位于另一 gateway 节点。

必须完成：

- 确保一个 match 同一时刻只有一个 frame relay owner。
- 输入消息按客户端单调序号丢弃陈旧值，帧历史、乱序缓冲和回调队列必须保持有界。
- 帧广播失败和客户端连续帧缺口必须有可观测日志，不能悄悄卡住模拟。
- 增加低频确定性状态摘要，检测不同客户端在同一帧产生不同状态，但不把摘要当作完整反作弊方案。
- 不能把实时 RPC callback 对象序列化进 Redis、PostgreSQL 或任何持久化状态。

验收门槛：

- 两个 gateway 实例运行时，玩家控制连接、实时连接和房间 actor owner 可以分布在不同网关。
- 玩家输入能到达房间 actor owner，输入帧能回到正确客户端并生成相同 world state。
- 关闭其中一个 gateway 后，受影响玩家得到明确失败/重连体验，其他房间不受影响。

### 清理与恢复语义

`PlayerService` 和会话生命周期已接入 `Lakona.Game.Server.Sessions`，可靠推送 ack 已接入框架 outcome。上线前仍必须集中验证这些路径、断线清理、frame relay periodic loop、帧重放和客户端结算上报的业务清理语义。
...
- 空房间可以停止 frame relay timer 并释放帧历史和会话回调资源。
...
- 当前服务器按产品要求不运行战斗计算，因此客户端排名报告本质上不可信。强对抗公网竞技上线前必须增加权威裁判、可验证状态证明或改为受信玩家场景；仅做字段校验和限流不能解决作弊。
...
- 可靠匹配推送重复到达时的幂等处理需要生产回归。
- 客户端 latest reliable sequence 已绑定到当前可靠推送会话；仍需回归确认新会话不会误 ack 旧会话序列。
- 对局中输入帧中断后的表现：短暂等待、重连重放、帧历史过期后返回大厅或重新登录。
...
1. 稳定玩家主流程：重点回归登录、匹配、进入对局、断线重连、结算和排行榜，保证已接入的会话可靠性能力在真实网络波动下不会让玩家卡在中间状态。
...
当前最大风险有四个：多服务器实时同步还未完成；断线、登出、取消匹配等异常流程虽然已有框架会话基础但需要继续生产回归；正式部署、监控和告警还不完整；账号凭据和请求校验需要达到外部上线的安全要求。只要这些问题没有关闭，就不建议直接对外生产上线。
