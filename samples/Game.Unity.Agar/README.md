...
```csharp
await actors.Route<RoomActor>(roomId).CallAsync(static behavior => behavior.JoinAsync, request);    // 默认业务路径，由 Route 负责查目录和选节点
await actors.Local<RoomActor>(roomId).PostAsync(static behavior => behavior.RunFrameAsync, request); // 已确认本地归属后，只在当前节点组帧并转发
await actors.Startup<MatchmakingActor>(queueId).CallAsync(static behavior => behavior.EnqueueAsync, request); // 按固定 selector 选择就绪副本
```
