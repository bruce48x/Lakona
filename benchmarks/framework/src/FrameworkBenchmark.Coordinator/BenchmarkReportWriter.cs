...
internal static class BenchmarkReportWriter
{
    public static void Write(string path, RunSummary summary)
    {
        ...
        builder.AppendLine();
        builder.AppendLine("| Workload | Payload | Concurrency | Framework | Valid | RPS | p50 us | p95 us | p99 us | Max us | Errors |");
        builder.AppendLine("| --- | ---: | ---: | --- | --- | ---: | ---: | ---: | ---: | ---: | --- |");
                     ...
                     .ThenBy(static item => item.Case.Framework, StringComparer.Ordinal))
        {
            builder.Append("| ").Append(item.Case.Workload)
                ...
                .Append(" | ").Append(Percentile(item.Result.Histogram, 0.50).ToString(CultureInfo.InvariantCulture))
