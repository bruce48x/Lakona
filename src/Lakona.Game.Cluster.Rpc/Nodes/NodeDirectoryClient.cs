...
{
    public sealed class NodeDirectoryClient : INodeDirectory
    {
        private readonly IRpcClient _client;

        public NodeDirectoryClient(IRpcClient client)
        {
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var reply = await _client.CallAsync(
                ClusterProtocol.RegisterNodeMethod,
            ...
            return new NodeRegistrationResult(
                ...
                NodeDirectoryRecordConverter.ToNodeRecord(reply.Record));
        }
        ...
        {
            var reply = await _client.CallAsync(
                ClusterProtocol.HeartbeatNodeMethod,
        ...
        {
            var reply = await _client.CallAsync(
                ClusterProtocol.UpdateNodeStateMethod,
        ...
        {
            var reply = await _client.CallAsync(
                ClusterProtocol.ResolveNodeMethod,
            ...
            CancellationToken cancellationToken = default)
        {
            ...
            var reply = await _client.CallAsync(
                ClusterProtocol.QueryNodesMethod,
                new NodeQueryRequest
                {
                    Query = NodeDirectoryRecordConverter.ToDto(query),
                    Now = now
                },
                cancellationToken).ConfigureAwait(false);
            ...
            {
                records.Add(NodeDirectoryRecordConverter.ToNodeRecord(reply.Records[i]));
            }
        ...
        {
            var reply = await _client.CallAsync(
                ClusterProtocol.ExpireNodesMethod,
                new NodeExpireRequest
                {
                    ClusterName = clusterName,
                    Now = now
                },
