...
public sealed class TypedActorSpawnGenerator : IIncrementalGenerator
{
    public void Initialize(IncrementalGeneratorInitializationContext context)
    {
        ...
        context.RegisterSourceOutput(actorTypes, static (ctx, actors) =>
        {
            ...
            ctx.AddSource("LakonaActorTypedActorSpawnExtensions.g.cs", SourceText.From(source, Encoding.UTF8));
        });
    }
    ...
    private static INamedTypeSymbol? GetTypedActorMessage(INamedTypeSymbol actorType)
    {
        ...
        {
            if (interfaceType is { Name: "IActor", Arity: 1 } &&
                interfaceType.ContainingNamespace.ToDisplayString() == "Lakona.Actor")
            {
    ...
    private static string GenerateSource(IReadOnlyList<INamedTypeSymbol> actorTypes)
    {
        ...
        source.AppendLine();
        source.AppendLine("namespace Lakona.Actor;");
        source.AppendLine();
        source.AppendLine("public static class LakonaActorTypedActorSpawnExtensions");
        source.AppendLine("{");
        ...
        foreach (INamedTypeSymbol actorType in actorTypes)
        {
            ...
            source.AppendLine($"    public static global::System.Threading.Tasks.ValueTask<global::Lakona.Actor.ActorHandle<{messageTypeName}>> {methodName}(");
            source.AppendLine("        this global::Lakona.Actor.ActorSystem system,");
            source.AppendLine($"        {actorTypeName} actor,");
            source.AppendLine("        global::Lakona.Actor.ActorSpawnOptions? options = null)");
            source.AppendLine("    {");
