...
public sealed class EndpointRule : ILakonaGameValidationRule
{
    ...
    public IEnumerable<LakonaGameDiagnostic> Validate(LakonaGameResolvedRuntime runtime)
    {
        ...
        foreach (var endpoint in runtime.Endpoints)
        {
            ...
            {
                yield return Error("LAKONA020", "Endpoint transport is required.", endpoint.Transport.Path);
            }
            else
            {
                if (!KnownTransports.Contains(transport))
                {
                    yield return Error("LAKONA020", $"Endpoint transport '{transport}' is unknown.", endpoint.Transport.Path, "Use kcp, tcp, or websocket.");
                }

                if (string.Equals(transport, "websocket", StringComparison.OrdinalIgnoreCase)
                    && string.IsNullOrWhiteSpace(endpoint.Path.Value))
                {
                    yield return Error("LAKONA023", "WebSocket endpoint requires Path.", endpoint.Path.Path, "Set Path to a path such as /ws.");
                }

                if (string.Equals(transport, "kcp", StringComparison.OrdinalIgnoreCase)
                    && !string.IsNullOrWhiteSpace(endpoint.Path.Value))
                {
                    yield return Error("LAKONA025", "KCP endpoint must not set Path.", endpoint.Path.Path, "Remove Path from the KCP endpoint.");
                }

                if (!transports.Add(transport))
                {
                    yield return Error("LAKONA024", $"Endpoint transport '{transport}' is configured more than once.", endpoint.Transport.Path);
                }
            }
            ...
            {
                yield return Error("LAKONA028", "Endpoint serializer is required.", endpoint.Serializer.Path);
            }
            ...
            {
                yield return Error("LAKONA021", "Endpoint host is required.", endpoint.Host.Path);
            }
            ...
            {
                yield return Error("LAKONA026", $"Endpoint bind address '{bind}' is configured more than once.", endpoint.Port.Path);
            }
