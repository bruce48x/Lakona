...
public sealed class LakonaRpcSourceGenerator : ISourceGenerator
{
    ...
    private static class ClientSourceEmitter
    {
            ...
            string generatedNamespace)
        {
            ...
            writer.Line("client.Runtime.RegisterRawNotificationHandler(GameSessionNotificationRpcIds.ServiceId, GameSessionNotificationRpcIds.EstablishedNotificationId, payload => _core.ApplyGameSessionEstablishedAsync(LakonaInternalCodec.DecodeGameSessionEstablished(payload)));");
            ...
            writer.Line("_ = RecoverAsync(ex);");
            writer.CloseBlock();
            writer.Line();
            writer.OpenBlock("private async Task RecoverAsync(Exception? disconnectReason)");
            writer.Line("var deadline = DateTimeOffset.UtcNow + _core.SessionResumeWindow;");
            writer.Line("var delay = TimeSpan.FromMilliseconds(100);");
            writer.OpenBlock("while (DateTimeOffset.UtcNow < deadline && !_lifetime.IsCancellationRequested)");
            writer.OpenBlock("try");
            writer.Line("await Task.Delay(delay, _lifetime.Token).ConfigureAwait(false);");
            ...
            writer.CloseBlock();
            writer.OpenBlock("catch");
            writer.Line("delay = TimeSpan.FromMilliseconds(Math.Min(delay.TotalMilliseconds * 2, 2000));");
            writer.CloseBlock();
            writer.CloseBlock();
