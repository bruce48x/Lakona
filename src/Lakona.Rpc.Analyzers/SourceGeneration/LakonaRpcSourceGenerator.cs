...
public sealed class LakonaRpcSourceGenerator : ISourceGenerator
{
    ...
    private static class ClientSourceEmitter
    {
            ...
            string generatedNamespace)
        {
            ...
            writer.Line("private readonly object[] _callbackReceivers;");
            writer.Line("private readonly ReconnectableRpcClient _dispatcher = new ReconnectableRpcClient();");
            writer.Line($"private readonly global::{generatedNamespace}.RpcApi _api;");
            writer.Line($"private global::{generatedNamespace}.RpcClient? _rpcClient;");
            writer.Line("private readonly CancellationTokenSource _lifetime = new CancellationTokenSource();");
            writer.Line("private readonly SemaphoreSlim _connectionGate = new SemaphoreSlim(1, 1);");
            writer.Line("private readonly object _recoveryGate = new object();");
            writer.Line("private Task? _recoveryTask;");
            ...
            writer.Line("_callbackReceivers = callbackReceivers;");
            writer.Line($"_api = new global::{generatedNamespace}.RpcApi(_dispatcher);");
            writer.CloseBlock();
            writer.Line();
            ...
            writer.CloseBlock();
            writer.Line("_apiReady = false;");
            writer.Line("_lifetime.Cancel();");
            writer.Line("Task? recoveryTask;");
            writer.OpenBlock("lock (_recoveryGate)");
            writer.Line("recoveryTask = _recoveryTask;");
            writer.CloseBlock();
            writer.OpenBlock("if (recoveryTask is not null)");
            writer.OpenBlock("try");
            writer.Line("await recoveryTask.ConfigureAwait(false);");
            writer.CloseBlock();
            writer.OpenBlock("catch (OperationCanceledException)");
            writer.CloseBlock();
            writer.CloseBlock();
            writer.Line("await _core.DisposeAsync().ConfigureAwait(false);");
            writer.OpenBlock("if (_rpcClient is not null)");
            writer.Line("await _rpcClient.DisposeAsync().ConfigureAwait(false);");
            writer.CloseBlock();
            writer.Line("_lifetime.Dispose();");
            writer.Line("_connectionGate.Dispose();");
            writer.CloseBlock();
            writer.Line();
            writer.OpenBlock("private GameClientHello CreateClientHello()");
            writer.OpenBlock("return new GameClientHello");
            writer.Line("ProtocolVersion = 1,");
            writer.Line("ResumeTicket = _core.ResumeTicket");
            writer.CloseBlock(";");
            writer.CloseBlock();
            writer.Line();
            writer.OpenBlock($"private async ValueTask ConnectGenerationAsync(bool recovering, CancellationToken ct)");
            ...
            writer.Line("var hello = await _core.HandshakeAsync(client.Runtime, CreateClientHello(), ct).ConfigureAwait(false);");
            writer.OpenBlock("if (recovering && hello.Recovery.Status != GameSessionRecoveryStatus.Resumed)");
            writer.Line("_core.ApplyRecoveryRejection(hello.Recovery);");
            writer.Line("throw new GameSessionRecoveryRejectedException(hello.Recovery.Status, hello.Recovery.Reason);");
            writer.CloseBlock();
            writer.Line("_core.BindReliablePush(client.Runtime);");
            writer.Line("client.Runtime.RegisterRawNotificationHandler(GameSessionNotificationRpcIds.ServiceId, GameSessionNotificationRpcIds.EstablishedNotificationId, async payload =>");
            writer.Line("{");
            writer.Line("    await _core.ApplyGameSessionEstablishedAsync(LakonaInternalCodec.DecodeGameSessionEstablished(payload)).ConfigureAwait(false);");
            ...
            writer.Line("_dispatcher.SetCurrent(client.Runtime);");
            writer.OpenBlock("if (recovering)");
            writer.Line("_core.MarkRecovered();");
            writer.CloseBlock();
            writer.OpenBlock("else");
            writer.Line("_core.MarkReady();");
            writer.CloseBlock();
            writer.Line("_apiReady = true;");
            writer.OpenBlock("if (previous is not null)");
            writer.Line("await previous.DisposeAsync().ConfigureAwait(false);");
            writer.CloseBlock();
            writer.CloseBlock();
            writer.OpenBlock("catch");
            writer.Line("await client.DisposeAsync().ConfigureAwait(false);");
            writer.Line("throw;");
            writer.CloseBlock();
            writer.CloseBlock();
            writer.OpenBlock("finally");
            writer.Line("_connectionGate.Release();");
            writer.CloseBlock();
            writer.CloseBlock();
            writer.Line();
            writer.OpenBlock($"private void HandleDisconnected(global::{generatedNamespace}.RpcClient source, Exception? ex)");
            writer.OpenBlock("if (Volatile.Read(ref _disposed) != 0)");
            writer.Line("return;");
            writer.CloseBlock();
            writer.OpenBlock("if (!ReferenceEquals(_rpcClient, source))");
            writer.Line("return;");
            writer.CloseBlock();
            writer.Line("_apiReady = false;");
            writer.Line("_dispatcher.ClearCurrent(source.Runtime);");
            writer.Line("_core.MarkReconnecting();");
            writer.OpenBlock("lock (_recoveryGate)");
            writer.OpenBlock("if (_recoveryTask is null || _recoveryTask.IsCompleted)");
            writer.Line("_recoveryTask = RecoverAsync(ex);");
            writer.CloseBlock();
            writer.CloseBlock();
            writer.CloseBlock();
            writer.Line();
            writer.OpenBlock("private async Task RecoverAsync(Exception? disconnectReason)");
            writer.Line("var scheduler = _options.RecoveryScheduler;");
            writer.Line("var deadline = scheduler.GetUtcNow() + _core.SessionResumeWindow;");
            writer.Line("var attempt = 0;");
            writer.OpenBlock("while (scheduler.GetUtcNow() < deadline && !_lifetime.IsCancellationRequested)");
            writer.OpenBlock("try");
            writer.Line("var remaining = deadline - scheduler.GetUtcNow();");
            writer.Line("var delay = scheduler.GetDelay(attempt++);");
            writer.OpenBlock("if (delay > remaining)");
            writer.Line("delay = remaining;");
            writer.CloseBlock();
            writer.Line("await scheduler.DelayAsync(delay, _lifetime.Token).ConfigureAwait(false);");
            writer.Line("await ConnectGenerationAsync(true, _lifetime.Token).ConfigureAwait(false);");
            writer.Line("return;");
            writer.CloseBlock();
            writer.OpenBlock("catch (OperationCanceledException) when (_lifetime.IsCancellationRequested)");
            writer.Line("return;");
            writer.CloseBlock();
            writer.OpenBlock("catch (GameSessionRecoveryRejectedException)");
            writer.Line("Disconnected?.Invoke(disconnectReason);");
            writer.Line("return;");
            writer.CloseBlock();
            writer.OpenBlock("catch");
            writer.CloseBlock();
            writer.CloseBlock();
            writer.Line("_core.ApplyRecoveryRejection(new GameSessionRecoveryHandshakeResult { Status = GameSessionRecoveryStatus.StateLost, Reason = \"Game session recovery window expired.\" });");
            writer.Line("Disconnected?.Invoke(disconnectReason);");
            writer.CloseBlock();
            writer.Line();
            ...
            if (notificationContracts.Count > 0)
            {
                ...
                writer.Line($"return new global::{generatedNamespace}.RpcClient(_options.CreateConnectionGeneration(), bindings);");
                writer.CloseBlock();
                writer.Line();
            ...
            else
            {
                writer.OpenBlock($"private global::{generatedNamespace}.RpcClient CreateRpcClient(object[] callbackReceivers)");
                writer.Line("ValidateCallbackReceivers(callbackReceivers);");
                writer.Line($"return new global::{generatedNamespace}.RpcClient(_options.CreateConnectionGeneration());");
                writer.CloseBlock();
                writer.Line();
            }
