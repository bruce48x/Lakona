...
public sealed class ClusterRpcMemoryPackGenerator : IIncrementalGenerator
{
    ...
    {
        var needsRpcPushMetadataFormatter = resolvedTypes
            .SelectMany(static type => type.Properties)
            .Any(static property => IsRpcPushMetadata(property.Symbol.Type));
        ...
        if (needsRpcPushMetadataFormatter)
        {
            builder.AppendLine("            global::MemoryPack.MemoryPackFormatterProvider.Register<global::Lakona.Rpc.Core.RpcPushMetadata>(new @RpcPushMetadataFormatter());");
        }
        ...
        if (needsRpcPushMetadataFormatter)
        {
            AppendRpcPushMetadataFormatter(builder);
        }

        return builder.ToString();
    }

    private static void AppendRpcPushMetadataFormatter(StringBuilder builder)
    {
        builder.AppendLine("file sealed class @RpcPushMetadataFormatter : global::MemoryPack.MemoryPackFormatter<global::Lakona.Rpc.Core.RpcPushMetadata>");
        builder.AppendLine("{");
        builder.AppendLine("    public override void Serialize<TBufferWriter>(ref global::MemoryPack.MemoryPackWriter<TBufferWriter> writer, scoped ref global::Lakona.Rpc.Core.RpcPushMetadata? value)");
        builder.AppendLine("    {");
        ...
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        var __field0 = value.Type;");
        builder.AppendLine("        var __field1 = value.Payload.ToArray();");
        builder.AppendLine("        writer.WriteObjectHeader((byte)2);");
        builder.AppendLine("        writer.WriteVarInt(writer.GetStringWriteLength(__field0));");
        builder.AppendLine("        writer.WriteVarInt(writer.GetUnmanageArrayWriteLength<byte>(__field1));");
        builder.AppendLine("        writer.WriteString(__field0);");
        builder.AppendLine("        writer.WriteUnmanagedArray(__field1);");
        builder.AppendLine("    }");
        builder.AppendLine();
        builder.AppendLine("    public override void Deserialize(ref global::MemoryPack.MemoryPackReader reader, scoped ref global::Lakona.Rpc.Core.RpcPushMetadata? value)");
        builder.AppendLine("    {");
        ...
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        if (count > 2)");
        builder.AppendLine("        {");
        builder.AppendLine("            global::MemoryPack.MemoryPackSerializationException.ThrowInvalidPropertyCount(typeof(global::Lakona.Rpc.Core.RpcPushMetadata), 2, count);");
        builder.AppendLine("        }");
        builder.AppendLine();
        ...
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        var __field0 = value is null ? string.Empty : value.Type;");
        builder.AppendLine("        var __field1 = value is null ? global::System.Array.Empty<byte>() : value.Payload.ToArray();");
        builder.AppendLine("        switch (count)");
        builder.AppendLine("        {");
        builder.AppendLine("            case 0:");
        builder.AppendLine("                break;");
        builder.AppendLine("            case 1:");
        builder.AppendLine("                if (deltas[0] != 0)");
        builder.AppendLine("                {");
        builder.AppendLine("                    __field0 = reader.ReadString();");
        builder.AppendLine("                }");
        builder.AppendLine("                break;");
        ...
        builder.AppendLine("                    __field0 = reader.ReadString();");
        builder.AppendLine("                }");
        builder.AppendLine("                if (deltas[1] != 0)");
        builder.AppendLine("                {");
        builder.AppendLine("                    __field1 = reader.ReadUnmanagedArray<byte>();");
        builder.AppendLine("                }");
        builder.AppendLine("                break;");
        builder.AppendLine("        }");
        builder.AppendLine();
        builder.AppendLine("        value ??= new global::Lakona.Rpc.Core.RpcPushMetadata();");
        builder.AppendLine("        value.Type = __field0 ?? string.Empty;");
        builder.AppendLine("        value.Payload = __field1 ?? global::System.Array.Empty<byte>();");
        builder.AppendLine("    }");
        builder.AppendLine("}");
        builder.AppendLine();
    }
    ...
    private static bool IsRpcPushMetadata(ITypeSymbol type)
    {
        type = UnwrapNullable(type);
        return type is INamedTypeSymbol namedType &&
            namedType.Name == "RpcPushMetadata" &&
            namedType.ContainingNamespace?.ToDisplayString() == "Lakona.Rpc.Core";
    }
