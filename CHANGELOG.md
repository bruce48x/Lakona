...
`Lakona.Rpc.Core 0.14.1`, `Lakona.Tool 0.43.3`, and `Lakona Hub 0.17.3`.
...
- Behavior entry methods now take only the Actor and request DTO. Remove their
  ...
  callers. Lifecycle and timer cancellation remain available. Cluster protocol `lakona.cluster.v5` removes cancellation messages and
  invocation ids. Upgrade cluster nodes together; older protocols are rejected.
- Unified diagnostic IDs as `LAKONA` plus five digits across runtime, Hotfix,
  RPC, and project generation. Update suppressions and health-code filters using
  the [diagnostic migration table](docs/guardrails.md#migration-from-previous-ids).
...
`Lakona.Game.Testing 0.2.9`, `Lakona.Game.Clustering.Postgres 0.1.8`,
`Lakona.Game.Clustering.Redis 0.1.8`,
`Lakona.Game.Clustering.MySql 0.1.8`, `Lakona.Tool 0.38.13`, and
`Lakona Hub 0.12.11`.
...
- Server-initiated termination now sends its fixed framework notification
  ...
  of application callback binders. Its atomic terminal transition also closes
  the exact connection that wins a concurrent recovery/rebind race.
...
- Actor Directory transition locks, recovery telemetry, bounded receiver-side
  Membership catch-up, deterministic Membership-view barriers, and five-node
  TestCluster stress coverage now exercise cancellation, failed handoffs,
  incarnation fencing, snapshots, churn, and partitioned joins without leaking
  transient view propagation as call failure.
