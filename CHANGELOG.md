...
- `Lakona.Tool` `0.8.24`: restored Unity CN and Tuanjie scaffolding to use the Unity renderer, force embedded NuGetForUnity, preserve OpenUPM metadata for standard Unity, extract the embedded NuGetForUnity package, and generate compose files that use the published `Server.App.dll` entrypoint.
